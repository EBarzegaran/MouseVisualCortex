function [AS_Wave, AS_STOK] = SpecEstimate(Data,freqs)
...
S_Wave      = xwt_cmorl_nv(permute(Y,[3 1 2]), Data.srate, freqs, 1, 6);
AS_Wave     = reshape(S_Wave,Nodes*Nodes,size(S_Wave,3),size(S_Wave,4));
...
AS_WaveN     = reshape(S_WaveN,Nodes*Nodes,size(S_WaveN,3),size(S_WaveN,4));
AS_WaveN     = AS_WaveN(1:Nodes+1:end,:,:); %diagonal spectrum
...
S_STOK      = abs(KF.SS.^2);
AS_STOK     = reshape(S_STOK,Nodes*Nodes,size(S_STOK,3),size(S_STOK,4));
AS_STOK     = AS_STOK(1:Nodes+1:end,:,:); %diagonal spectrum
% normalize, to minimize 1/f noise
S_STOKN     = S_STOK - mean(S_STOK(:,:,:,(Times<0)& (Times>-.3)),4);
% keep the autospectrum
AS_STOKN     = reshape(S_STOKN,Nodes*Nodes,size(S_STOKN,3),size(S_STOKN,4));
AS_STOKN     = AS_STOKN(1:Nodes+1:end,:,:); %diagonal spectrum
% open up some space in the memory
