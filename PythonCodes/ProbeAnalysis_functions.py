...
def extract_probeinfo(session, lfp, probe_id, Resultspath, doRF):
    ...
    # do RF mapping
    if doRF:
        rf_results = RF_mapping_plot(session, lfp, probe_id, False, Resultspath)
    else:
        rf_results = []

    # Save the result
    sio.savemat(os.path.join(Resultspath, 'MatlabData', '{}_ProbeInfo_RF.mat'.format(probe_id)),
...
def prepare_condition(session, lfp, probe_id, cond_name, Resultspath, Prestim, down_rate):
    ...
    # save data
    if not os.path.exists(os.path.join(Resultspath, 'MatlabData')):
        os.mkdir(os.path.join(Resultspath, 'MatlabData'))

    sio.savemat(os.path.join(Resultspath, 'MatlabData', '{}_{}{}.mat'.format(probe_id, cond_name, int(dSRate))),
