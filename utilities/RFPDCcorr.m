function Results = RFPDCcorr(StokALL,P_th,Path,CondName)
    ...
    for id = 1:numel(IDs)
        ...
        Dist_temp   = reshape(StokALL.(IDs{id}).ProbeRFDist,NNode*NNode,1);
    ...
    caxis([-.2 .2])
    colorbar;
    title('V1 out PDC correlation with RF distance')
    %export_fig(FIG,fullfile(Path,[CondName '_V1_out_RFCorr']),'-pdf','-r200');
    ...
    caxis([-.2 .2])
    colorbar;
    title('Golbal PDC correlation with RF distance')
    export_fig(FIG,fullfile(Path,[CondName '_Global_RFCorr']),'-pdf','-r200');
    ...
    %caxis([-.2 .2])
    colorbar;
    title('inter-area PDC correlation with RF distance')
    export_fig(FIG,fullfile(Path,[CondName '_InterArea_RFCorr']),'-pdf','-r200');
    %% plot laminar effect
    PDC_All_BR = PDC_All- mean(PDC_All(:,:,StokALL.S766640955.Times<0 & StokALL.S766640955.Times>-.6),3);
    %
    for t = 1:size(PDC_All_BR,3)
    ...
    xlim([0 2])
    colormap('jet')
    title('Laminar PDC correlation with RF distance')
    %caxis([-.5 -.05])
    colorbar;
    export_fig(FIG,fullfile(Path,[CondName '_Laminar_RFCorr']),'-pdf','-r200');
end